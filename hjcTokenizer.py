        ...
        while len(self.line):     # Characters remain
            ...
            if ch in identStart:
                ...
                if token_str in keywords:
                    self.token = (TK_KEYWORD, token_str)
                else:
                    self.token = (TK_IDENTIFIER, token_str)
                self.line = self.line[len(token_str):]
                return
            ...
            if ch in numberStart:
                ...
                for c in self.line[1:]:
                    if c in numberChars:
                        ...
                        break
                self.token = (TK_INT_CONST, token_str)
                self.line = self.line[len(token_str):]
                return
    ...
    def _ParseString(self):
        ...
        if stringStart == '"':
            ...
            for c in self.line:
                if c == '"':
                    parseString += c
                    self.line = self.line[len(parseString) + 1:]
                    return parseString
                else:
                    parseString += c
